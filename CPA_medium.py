...
# 计算相关性函数
def compute_correlation(plain_sample, leakage_sample):
    # 假设密钥K的范围
    ...
    R = np.zeros((len(K), tr_length))

    for key_idx in range(len(K)):
        for k in range(tr_length):
            R[key_idx, k] = pearsonr(H[:, key_idx], leakage_sample[:, k])[0]

    # 找到相关性最大的密钥
    mean_R = np.mean(R, axis=1)
    guessed_key = np.argmax(mean_R)  # 猜测的密钥
...
def perform_attack(m, n):
    ...
    guessed_key = compute_correlation(plain_sample, leakage_sample)
    return guessed_key
